...
class GTCLoss_sep(nn.Module):
    ...
    def __init__(self,
                 ...
                 zero_infinity=True,
                 **kwargs):
        super(GTCLoss_sep, self).__init__()
        # 动态构建CTCLoss
        ctc_config = {'name': 'CTCLoss', 'zero_infinity': zero_infinity}
        self.ctc_loss = build_loss(ctc_config)

        ctc_w_config = {'name': 'CTCLoss_sep', 'zero_infinity': zero_infinity}
        self.ctc_w_loss = build_loss(ctc_w_config)
        # 构建GTC损失
        self.gtc_loss = build_loss(gtc_loss)
        self.gtc_weight = gtc_weight
        self.ctc_weight = ctc_weight
        self.ctc_w_weight = ctc_w_weight

    def forward(self, predicts, batch):
        ctc_w_loss = self.ctc_w_loss(predicts['ctc_w_pred'],
                                 [None] + batch[-2:])['loss']
        

        ctc_loss = self.ctc_loss(predicts['ctc_pred'],
                                 [None] + batch[-4:-2])['loss']
        gtc_loss = self.gtc_loss(predicts['gtc_pred'], batch[:-4])['loss']
        return {
            'loss': self.ctc_weight * ctc_loss + self.gtc_weight * gtc_loss + self.ctc_w_weight * ctc_w_loss,
            'ctc_loss': ctc_loss,
            'gtc_loss': gtc_loss,
            'ctc_w_loss': ctc_w_loss,
        }